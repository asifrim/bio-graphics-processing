...
generic_track = my_panel.add_track('generic', :label => true, :colour => [1,0,1])
box_track = my_panel.add_track('box', :label => true, :glyph => :box)
...
triangle_track = my_panel.add_track('triangle', :label => true, :glyph => :triangle, :colour => [1,0,0])
spliced_track = my_panel.add_track('spliced', :label => true, :glyph => :spliced, :colour => [1,0,0])
directed_spliced_track = my_panel.add_track('directed_spliced', :label => true, :glyph => :directed_spliced, :colour => [1,0,1])
composite_track = my_panel.add_track('composite_features', :label => true, :glyph => { 'utr5' => :line, 'utr3' => :line, 'cds' => :directed_spliced})
transcript_track = my_panel.add_track('transcripts', :label => true, :glyph => :transcript)

generic_track.add_feature(Bio::Feature.new('clone', '250..375'), :label => 'clone1', :link => 'http://www.newsforge.com', :colour => [1,0.2,0])
...
generic_track.add_feature(Bio::Feature.new('clone', '54..124'), :label => 'clone4', :link => 'http://www.thearkdb.org')
generic_track.add_feature(Bio::Feature.new('clone', '100..449'), :label => 'clone5', :link => 'http://www.google.com')
...
box_track.add_feature(Bio::Feature.new('clone', '250..375'), :link  => 'http://www.newsforge.com',:colour => [1,1,0])
...
Zlib::GzipWriter.open('outputfile.gz') do |gz|
  gz.write Marshal.dump(my_panel)
end
